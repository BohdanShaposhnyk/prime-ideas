...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setFontsLoaded(true), 0);
    ...
    () => {
      ...
      const teardown = () => {
        ...
        try {
          ...
          /* SplitText revert can throw if already cleaned up */
        }
      ...
      const attrAt = (name: string) => (_index: number, node: HTMLElement) => {
        void _index
        return parseFloat(node.getAttribute(name) || '0')
      }
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, { y: attrAt('data-start-y') });
            } else {
              gsap.set(strips, { x: attrAt('data-start-x') });
            }
            onShuffleComplete?.();
          },
          onComplete: () => {
        ...
        const addTween = (targets: HTMLElement[], at: number) => {
          ...
          if (isVertical) {
            vars.y = attrAt('data-final-y');
          } else {
            vars.x = attrAt('data-final-x');
          }
