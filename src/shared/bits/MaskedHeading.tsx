...
const MaskedHeading: FC<MaskedHeadingProps> = ({
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${Math.max(20, root.clientWidth * s.textScale).toFixed(1)}px`;
  ...
  return (
    ...
    >
      ...
      <svg className="pointer-events-none absolute inset-0 size-full overflow-visible" aria-hidden="true" focusable="false">
        <defs>
          <clipPath id={clipId} clipPathUnits="userSpaceOnUse" data-mh-glyphs>
            {words.map((word, i) => (
              <text
                key={`${word}-${i}`}
