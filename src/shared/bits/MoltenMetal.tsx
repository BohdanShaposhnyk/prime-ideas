...
export interface MoltenMetalProps {
  ...
  lightMode?: boolean
  /** Internal buffer scale. 1 = full res. Default 0.55. */
  renderScale?: number
  /** Cap on devicePixelRatio. Default 1.5. */
  maxDpr?: number
  /** Animation frame budget. Default 30. */
  targetFps?: number
  className?: string
}
...
const MoltenMetal: React.FC<MoltenMetalProps> = ({
  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      if (contextLost || !isVisible || !isPageVisible) {
        raf = 0
        return
      }
    ...
    // Init once; prop patches live in the second effect.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [renderScale, maxDpr, targetFps, mouseInteraction])
