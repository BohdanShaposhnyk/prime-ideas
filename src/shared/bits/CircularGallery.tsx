...
/* eslint-disable @typescript-eslint/no-explicit-any -- vendor Bit */
...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: 'right' | 'left') {
    ...
    if (this.hideCenter) {
      const hole = Math.max(this.plane.scale.x * 1.05, this.viewport.width * 0.2);
      const t = Math.abs(x) / hole;
      this.program.uniforms.uOpacity.value = Math.min(1, Math.max(0, (t - 0.28) / 0.42));
      if (this.title?.mesh) this.title.mesh.visible = this.program.uniforms.uOpacity.value > 0.2;
    } else {
