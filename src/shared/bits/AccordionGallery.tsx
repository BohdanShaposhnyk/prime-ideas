...
export interface AccordionGalleryProps {
  ...
  /** Fill the parent box instead of a fixed pixel height. */
  fillParent?: boolean;
  /** Extra dim on inactive panels (0–1). */
  inactiveDim?: number;
  /** Dim on the active panel (0–1) — useful when overlaying UI. */
  activeDim?: number;
  /** Renders inside the active panel, above the image. */
  renderPanelContent?: (index: number) => ReactNode;
}
...
const AccordionGallery = ({
  ...
  const sizeStyle: CSSProperties = fillParent
    ? { gap: `${gap}px`, height: '100%', width: '100%' }
    : { gap: `${gap}px`, height: vertical ? `${Math.round(height * 1.6)}px` : `${height}px` };
      ...
      {items.map((item, i) => {
        ...
        return (
          ...
          >
            ...
            {!isActive && item.label ? (
              <span
                className="pointer-events-none absolute inset-0 z-[2] flex items-center justify-center overflow-visible px-1"
                aria-hidden="true"
              >
                <span
                  className={`whitespace-nowrap font-[family-name:var(--cs-display,ui-sans-serif)] font-normal uppercase [text-shadow:0_2px_14px_rgba(0,0,0,0.55)] ${
                    vertical
                      ? 'max-w-[calc(100%-0.5rem)] overflow-hidden text-ellipsis text-[clamp(0.9rem,1.6vw,1.25rem)] tracking-[0.14em]'
                      : 'origin-center -rotate-90 text-[clamp(1.15rem,2.4vw,1.85rem)] tracking-[0.16em] max-[520px]:max-w-[calc(100%-0.75rem)] max-[520px]:rotate-0 max-[520px]:overflow-hidden max-[520px]:text-ellipsis max-[520px]:text-[clamp(0.9rem,3.6vw,1.15rem)] max-[520px]:tracking-[0.12em]'
                  }`}
                  style={{ color: textColor }}
                >
                  {item.label}
            ...
            {isActive && renderPanelContent ? (
              <span
                className="absolute inset-0 z-[3] flex flex-col p-4 sm:p-5 md:p-6"
