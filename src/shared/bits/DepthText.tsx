...
const DepthText = ({
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const orbiting = autoOrbit && (!canTrackPointer || !activePointer);
      if (orbiting) {
      ...
      const settling =
        Math.abs(target.x - current.x) > settledEpsilon || Math.abs(target.y - current.y) > settledEpsilon;
      if (orbiting || activePointer || settling) {
