import type { CSSProperties } from 'react'

const tokens = {
  '--gc-mist': '#E8EEF2',
  '--gc-slate': '#1B2A33',
  '--gc-aqua': '#7EB8C9',
  '--gc-clear': '#F7F4EF',
  '--gc-brass': '#C9A87C',
  '--gc-ember': '#D4784A',
  '--gc-ink': '#0F171C',
} as CSSProperties

/**
 * Kitchen — heat blooms under the lift.
 */
export default function SceneKitchen() {
  return (
    <section
      id="kitchen"
      aria-labelledby="scene-kitchen-title"
      data-scene="kitchen"
      data-scroll="bloom"
      className="sticky top-0 isolate flex h-dvh w-full overflow-hidden text-[var(--gc-clear)]"
      style={{ ...tokens, zIndex: 2 }}
    >
      {/* swap: kitchen heat plane */}
      <div data-placeholder="visual" className="absolute inset-0" aria-hidden>
        <div
          data-plane="under-cap"
          className="absolute inset-0"
          style={{
            background: `
              radial-gradient(58% 48% at 48% 58%, color-mix(in srgb, var(--gc-ember) 48%, transparent) 0%, transparent 62%),
              radial-gradient(42% 38% at 78% 40%, color-mix(in srgb, var(--gc-brass) 35%, transparent) 0%, transparent 55%),
              linear-gradient(165deg, #2A221C 0%, #1C1612 48%, var(--gc-ink) 100%)
            `,
          }}
        />
        <div
          data-plane="pass"
          className="absolute left-[10%] top-[44%] h-[14%] w-[70%] opacity-75 sm:left-[18%] sm:w-[52%]"
          style={{
            background: `
              linear-gradient(
                90deg,
                transparent 0%,
                color-mix(in srgb, var(--gc-ember) 50%, transparent) 30%,
                color-mix(in srgb, var(--gc-brass) 65%, white) 52%,
                transparent 100%
              )
            `,
            filter: 'blur(10px)',
          }}
        />
        <div
          data-plane="grain"
          className="absolute inset-0 opacity-[0.24] mix-blend-soft-light"
          style={{
            backgroundImage: `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.85' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.5'/%3E%3C/svg%3E")`,
            backgroundSize: '150px 150px',
          }}
        />
      </div>

      <div className="relative z-10 flex h-full w-full flex-col items-center justify-end px-5 pb-16 pt-[min(30vh,11rem)] text-center sm:px-8 sm:pb-20 lg:px-12">
        <p className="mb-3 font-['Figtree',sans-serif] text-[0.65rem] tracking-[0.22em] text-[color-mix(in_srgb,var(--gc-brass)_80%,var(--gc-mist))] uppercase">
          01 · Kitchen
        </p>
        <h2
          id="scene-kitchen-title"
          className="max-w-lg font-['Unbounded',sans-serif] text-[clamp(2.25rem,9vw,4rem)] leading-[0.95] font-medium tracking-[-0.03em] text-[color-mix(in_srgb,var(--gc-clear)_92%,var(--gc-ember))]"
        >
          Heat under glass
        </h2>
        <p className="mt-4 max-w-sm font-['Figtree',sans-serif] text-sm leading-relaxed text-[color-mix(in_srgb,var(--gc-mist)_58%,var(--gc-brass))] sm:text-base">
          Plates warm. The seal softens.
        </p>
      </div>
    </section>
  )
}
