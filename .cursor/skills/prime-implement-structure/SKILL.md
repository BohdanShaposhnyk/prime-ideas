...
description: >-
  ...
  Structure and Scroll grammar (scroll skeleton + section files + page compose,
  status structure). Use when the user asks for prime-implement-structure or
  to scaffold candidate sections.
disable-model-invocation: true
---

# prime-implement-structure

Scaffold the **scroll skeleton** and section shells after hero exists. Stop — do not flesh sections or install Bits.
...
- Refuse `c00-dry-run`
- Confirm target candidate; read CONCEPT **Scroll grammar** + Structure (ordered list)

## Steps

1. **Scroll skeleton** — Compose `index.tsx` as the container the Scroll grammar needs (sticky pin/zoom stage, horizontal track, dive scale wrapper, diagonal clip stage, vertical stack, etc.). CSS first; semantic hooks for later GSAP. Vertical `min-h` stack only if CONCEPT says so.
...
3. **Shells** — Default export each section: landmark, short title, one supporting line from CONCEPT — layout within the shell follows Layout grammar / that beat, not a default “title at bottom of gradient.”
   ...
   - No deep interaction chrome, no Bits, no GSAP timelines in `motion/`.
...
- Page implements the CONCEPT scroll skeleton (not a forced default vertical list)
- Status is `structure`
